...
def background_remover_and_bbox(image_files, padding=30):
    img_with_boxes_list = []
    ...
    for image_file in image_files:
        ...
        img = Image.open(image_file)  # 파일 객체를 이미지로 엽니다.

        base_name = os.path.basename(image_file.name)  # 이미지 원본 이름 추출
